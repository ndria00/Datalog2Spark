                    ...
                    with open(f"{output_folder_path}/{folder}/{partition}") as input_file:
                        for line in input_file: 
                            ...
                            for i in range(len(terms)):
                                terms[i] = truncate_number(terms[i])
                            tuple : str = ",".join(terms)
...
def print_tuples(clingo_tuples, spark_tuples):
    print("Tuples produced by clingo")
    for t in clingo_tuples:
        print(t, sep=" ")
    print("Tuples produced by spark")
    for t in clingo_tuples:
        print(t, sep=" ")
...
def main():
    ...
    if folders:
        for folder in folders:
            ...
            compile_command : str = f"./output/main ../{encoding_folder}/{folder}/{folder}Spark.lp"
            ...
            for instance in instances:
                ...
                execute_clingo_command : str = f"clingo {encoding_folder}/{folder}/{folder}Clingo.lp {instance_folder}/{folder}/{instance}/{input_clingo_folder}/*"
                ...
                if len(clingo_output) != len(spark_output):
                    #print_tuples(clingo_output, spark_output)
                    print(f"Test failed on instance {folder}/{instance}")
                    exit(1)
                    
                for tuple in spark_output:
                    if not tuple in clingo_output:
                        print(f"Test failed on instance {folder}/{instance} - tuple {tuple} was not generated by clingo")
                        #print_tuples(clingo_output, spark_output)
                        exit(1)
                for tuple in clingo_output:
                    if not tuple in spark_output:
                        print(f"Test failed on instance {folder}/{instance} - tuple {tuple} was not generated by spark")
                        #print_tuples(clingo_output, spark_output)
                        exit(1)
